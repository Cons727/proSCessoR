...
# Date:    2019.05.15
# Modified: 2023.12.08 (Constanza Marini; start from AIRR, omit Germlines)
#
# Arguments:
#   -a  IgBLAST AIRR output.
...
while getopts "a:t:x:m:e:d:u:n:o:f:p:izh" OPT; do
    case "$OPT" in
    a)  AIRR=$OPTARG
        AIRR_SET=true
        ;;
...
# Set output name
if ! ${OUTNAME_SET}; then
    OUTNAME=$(basename ${AIRR} | sed 's/\.[^.]*$//; s/_L[0-9]*_R[0-9]_[0-9]*//')
fi
...
# If failed: Commente out because absolute path contains spaces and couldn't figure out a solution. Failed in -w
    printf -v PARENTDIR %q "$(realpath ${OUTDIR})"
...
ParseDb.py select -d "${AIRR}" -f ${LOCUS_FIELD} -u IGH TRB TRD \
...
# if -TRUE to -T.
# Sometimes ParseDb.py saves output with one or the other format.
# Renaming files for consistency
if [ ! -e "$HEAVY_PROD" ]; then
    mv "${OUTNAME}_heavy_${PROD_FIELD}-TRUE.${EXT}" "$HEAVY_PROD"
fi

if [ ! -e "$LIGHT_PROD" ]; then
    mv "${OUTNAME}_light_${PROD_FIELD}-TRUE.${EXT}" "$LIGHT_PROD"
fi
...
if $CLONE; then
    ...
    if [ "$DIST" == "auto" ]; then
        ...
        if [ $THRESHOLD_METHOD == "gmm" ]; then
            DIST=$(tail -n1 "${OUTNAME}_threshold-values.tab" | cut -f13)
        else
