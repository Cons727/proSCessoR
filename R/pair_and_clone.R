...
#'
#' This function acts as a wrapper to execute the bash-based pairing and clonotyping pipeline, handling module loads, fasta merging, and IgBLAST alignment.
#'
...
#' @param python_module Character. Python libraries where all python dependancies are installed for example "Python/3.8.2-GCCcore-9.3.0".
#' 
#' @return Invisible integer of the system call exit status (0 means success).
#' @export

pair_and_clone <- function(outname,
                        ref,
                        db_name = "ogrdb",
                        vdj = "vdj_b",
                        fasta = NULL,
  ...
  script_path <- file.path(pkg_bin_dir, "sbatch_pair_clone.sh")
  ...
  Sys.setenv(OUTNAME = outname,
             ...
             USER_PYTHON_MODULE = python_module,
             PATH = new_path) # The scripts should now find each other natively
  
  message("Starting pairing and clonotyping pipeline for: ", outname)
