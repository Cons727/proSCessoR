...
# Run IgSADIE and merge results with metadata. No clonotype analysis
# Example run:
# sbatch -c 16 --mail-user=cmarinim@fredhutch.org -o prSCR-40.3.log -J prSCR-40.3 --mail-type=END,FAIL --export=OUTNAME=HVTN301_BCS2040,REF=/fh/fast/_VIDD/HVTN/Vaccine/B_cell_Sequencing/OGRDB_ref/ref-2024.01.11/airr_c_human,DB_NAME=ogrdb,VDJ=vdj_b,FASTA=Processing_QC/HVTN301_BCS2040_singlets_prod_contig.fasta sbatch_pair_merge.sh
...
# Capture the pristine R library path passed down from the wrapper
...
ml BBMap IgBLAST/1.22.0-x64-linux 
...
# Gene Assignment with IgBLAST and input ref
CORES="$(nproc)"
...
# No clonotype analysis

merge_md_SADIE.R $OUTNAME